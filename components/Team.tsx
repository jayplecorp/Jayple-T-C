  ...
  return (
    <section id="team" className="section-padding bg-surface-off" aria-labelledby="team-heading">
      <div className="container-max">
        ...
        >
          <span className="text-xs font-bold uppercase tracking-widest text-text-muted mb-3 block">Our Organization</span>
        ...
        <motion.div
          className="grid grid-cols-2 lg:grid-cols-4 gap-5 max-w-3xl mx-auto mb-10"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: '-60px' }}
          variants={staggerContainer}
        >
          {MEMBERS.map(({ name, role, photo, initials }) => (
            <motion.div
              key={name}
              variants={fadeUp}
              transition={TRANSITION_DEFAULT}
            ...
            >
              {/* Avatar */}
              <div className="w-20 h-20 rounded-xl overflow-hidden mb-3 bg-navy shadow-md">
                {photo ? (
                  <Image src={photo} alt={name} width={80} height={80} className="object-cover w-full h-full" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center font-display font-extrabold text-xl text-lime">
                    {initials}
                  </div>
                )}
              </div>
              <h4 className="font-bold text-[15px] text-text">{name}</h4>
